...
export async function onRequest(context) {
  ...
  if (request.method !== "GET" && request.method !== "HEAD") {
    ...
    // base64 image upload) get dropped mid-flight. That's why a client signed in on
    // their proxied /admin could save text edits (tiny payloads) but not upload
    // photos. Reading it into an ArrayBuffer sends the whole payload as one
    // complete, correctly-sized request so uploads go through reliably.
